package com.dalao.yiban.constant;

public class MineConstant {

    public static final int EDIT_USER_NICKNAME_REQUEST_CODE = 1;

    public static final int SECRET = 0;

    public static final int MALE = 1;

    public static final int FEMALE = 2;

    public static final int SECRET_RESPONSE = 0;

    public static final int MALE_RESPONSE = 1;

    public static final int FEMALE_RESPONSE = 2;

    public static final int EDIT_NICKNAME = 0;

    public static final int EDIT_SEX = 1;

    public static final String SECRET_TEXT = "保密";

    public static final String MALE_TEXT = "男";

    public static final String FEMALE_TEXT = "女";

    public static final String USER_NICKNAME = "user_nickname";

    public static final String EDIT_USER_INFO_SUCCESS = "success";

    public static final String EDIT_USER_INFO_ERROR = "error";

}
